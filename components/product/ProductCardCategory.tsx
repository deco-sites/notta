...
function ProductCardCategory(
  ...
  const imageNoMeasurements = images?.filter((image) =>
    image.name !== "medida"
  );

  const front = index?.toString() && images ? images[index] : undefined;
  const [back] = images ?? [];

  const { listPrice, price = 0, installments, seller = "1" } = useOffer(offers);
  ...
  return (
    ...
    >
      ...
      {/* Prices & Name */}
      <div class="flex-auto flex flex-col relative h-16  gap-1 lg:gap-2">
        {/* SKU Selector */}
        {
